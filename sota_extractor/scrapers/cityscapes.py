...
CITYSCAPES_URL = "https://www.cityscapes-dataset.com/benchmarks/#pixel-level-results"
    ...
    for row in rows:
        ...
        if len(cells) == 24:
            ...
            if row.find('td', attrs={"class": "column-21"}).a:
                paper_url = row.find('td', attrs={"class": "column-21"}).a['href']
            else:
                paper_url = None
            ...
            if paper_title and class_iou and category_iou:
                sota_row = SotaRow(
                        ...
                        paper_url=paper_url,
                        paper_date=None,
                        metrics={"Mean IoU (class)": class_iou,
...
def cityscapes():
    ...
    if len(sota_tabels) == 3:
        ...
        task.source_link = Link(title="CityScapes Leaderboard", url=CITYSCAPES_URL)
